...
interface MessageComposerProps {
  ...
  /** True when the contact has opted out (replied STOP/UNSUBSCRIBE) —
   *  every outbound send path enforces this server-side regardless, but
   *  disabling the composer here avoids a round trip just to find out. */
  optedOut?: boolean;
  ...
  onClearReply?: () => void;
  /** Seeds the composer's text — used by the AI Copilot draft banner's
   *  "Edit" action. Only re-applied when `prefillKey` changes (not on
   *  every render), so it doesn't fight the user's own typing. */
  prefillText?: string;
  prefillKey?: string;
}
...
export function MessageComposer({
  ...
  useEffect(() => {
    ...
    // Intentionally keyed on prefillKey only — re-running this whenever
    // prefillText's identity changes (e.g. a parent re-render) would
      ...
      {optedOut && (
        <div className="mb-2 rounded-lg bg-destructive/10 px-3 py-2">
          <p className="text-xs text-destructive">
            This contact opted out of messages (replied STOP). They must text START to resume
            before you can message them again.
          </p>
        </div>
      )}
      ...
      {draft ? (
        ...
        <div className="flex items-end gap-2">
          ...
          <textarea
            ...
            placeholder={
              readOnly
                ...
                : optedOut
                  ? "Contact opted out — they must text START to resume"
                  : sessionExpired
                    ? "Session expired - use a template"
                    : "Type a message... (Shift+Enter for new line)"
            }
            disabled={sessionExpired || optedOut || readOnly}
            rows={1}
            ...
            className={cn(
              ...
              (sessionExpired || optedOut || readOnly) && "cursor-not-allowed opacity-50"
            )}
          />
          ...
          <GatedButton
            ...
            canAct={!readOnly}
            gateReason="send messages"
            disabled={!text.trim() || sessionExpired || optedOut || sending}
            onClick={handleSend}
