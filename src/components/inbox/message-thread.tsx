...
import { AiDraftBanner } from "./ai-draft-banner";
import { AiPendingActionBanner } from "./ai-pending-action-banner";
...
interface MessageThreadProps {
  ...
  /** Governs whether the AI agent replies at all — see ConversationMode.
   *  Optional so existing callers (and tests) keep working; the
   *  dropdown only renders when this is provided. */
  onModeChange?: (conversationId: string, mode: ConversationMode) => void;
  /**
...
/** Governs whether the AI agent replies at all — orthogonal to
 *  STATUS_OPTIONS above. See the ConversationMode type. */
const MODE_OPTIONS: { label: string; value: ConversationMode; color: string }[] = [
  { label: "AI Active", value: "ai_active", color: "text-primary" },
  { label: "Human", value: "human_active", color: "text-amber-400" },
  { label: "AI Copilot", value: "ai_suggestion_only", color: "text-violet-400" },
  { label: "Paused", value: "paused", color: "text-muted-foreground" },
];
...
export function MessageThread({
  ...
  // AI Copilot draft banner — "Edit" seeds the composer with the
  // draft's text via prefillKey/prefillText; draftRefreshToken forces
  // the banner to refetch after a review action or a new inbound
  // message (a fresh AI draft may have just landed).
  const [composerPrefill, setComposerPrefill] = useState<{ key: string; text: string } | null>(
    null,
  );
  const [draftRefreshToken, setDraftRefreshToken] = useState(0);
  // Same refetch trigger, for the consequential-action approval banner
  // (see AiPendingActionBanner) — a new inbound message may mean the AI
  // just queued an action for approval.
  const [pendingActionRefreshToken, setPendingActionRefreshToken] = useState(0);
  // A new inbound message may mean the AI just wrote a fresh draft —
  // re-check whenever the message list changes.
  useEffect(() => {
    setDraftRefreshToken((t) => t + 1);
    setPendingActionRefreshToken((t) => t + 1);
  }, [messages.length]);
  ...
  const handleModeChange = useCallback(
    async (mode: ConversationMode) => {
      ...
      onModeChange?.(conversation.id, mode);
    },
  ...
  const currentMode = MODE_OPTIONS.find((m) => m.value === conversation.mode);
    ...
    <div className={cn("flex min-w-0 flex-1 flex-col", DOODLE_BG_CLASSES)}>
      ...
      <div className="flex items-center justify-between gap-2 border-b border-border bg-card px-3 py-3 sm:px-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* AI mode dropdown — only rendered when the parent wired
              onModeChange, mirroring the onRefresh pattern above.
              Governs whether the AI agent replies at all; independent
              of the Status dropdown next to it. */}
          {onModeChange && (
            <DropdownMenu>
              <DropdownMenuTrigger
                className={cn(
                  "inline-flex items-center justify-center h-7 gap-1 px-2 text-xs rounded-md hover:bg-muted",
                  currentMode?.color ?? "text-muted-foreground"
                )}
              >
      ...
      {/* AI Copilot draft — only meaningful in ai_suggestion_only mode,
          but the banner itself no-ops (renders nothing) whenever there's
          no pending draft, so it's safe to always mount. */}
      {conversation.mode === "ai_suggestion_only" && (
        <AiDraftBanner
          conversationId={conversation.id}
          refreshToken={draftRefreshToken}
          onResolved={() => setDraftRefreshToken((t) => t + 1)}
          onEdit={(text) =>
            setComposerPrefill({ key: `${conversation.id}-${Date.now()}`, text })
          }
        />
      )}

      {/* Consequential-action approval — applies regardless of
          conversation mode (gating is a per-account config toggle, not
          tied to Copilot mode), so always mounted; no-ops when nothing's
          pending. */}
      <AiPendingActionBanner
        conversationId={conversation.id}
        refreshToken={pendingActionRefreshToken}
        onResolved={() => setPendingActionRefreshToken((t) => t + 1)}
      />
      ...
      <MessageComposer
        ...
        prefillKey={composerPrefill?.key}
      />
