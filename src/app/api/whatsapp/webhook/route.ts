...
async function processMessage(
  ...
  // ============================================================
  // Opt-out / opt-in compliance keywords — checked before ANY
  // deterministic or AI dispatch below, and unconditionally short-
  // circuits the rest of this function when matched. STOP-style
  // keywords must be honored regardless of what flow/automation/AI
  // state the conversation is in; a flow mid-collect_input or an
  // active automation must not swallow it as ordinary input. See
  // src/lib/whatsapp/opt-out.ts for the matching rules and the full
  // list of send paths this then gates.
  // ============================================================
  const optKeywordText = contentText ?? message.text?.body ?? ''
  if (message.type === 'text' && optKeywordText) {
    const alreadyOptedOut = Boolean((contactRecord as { opted_out_at?: string | null }).opted_out_at)
    if (isOptOutMessage(optKeywordText) && !alreadyOptedOut) {
      await supabaseAdmin()
        .from('contacts')
        .update({ opted_out_at: new Date().toISOString() })
        .eq('id', contactRecord.id)
        .eq('account_id', accountId)
      await sendOptStatusConfirmation({
        accountId,
        conversationId: conversation.id,
        contactPhone: contactRecord.phone,
        text: "You've been unsubscribed and won't receive further messages from us. Reply START to resume.",
      })
      return
    }
    if (isOptInMessage(optKeywordText) && alreadyOptedOut) {
      await supabaseAdmin()
        .from('contacts')
      ...
      await sendOptStatusConfirmation({
        ...
        conversationId: conversation.id,
        contactPhone: contactRecord.phone,
        text: "You're resubscribed and will receive messages from us again. Welcome back!",
      })
      return
    }
  }
  ...
  // ============================================================
  // Conversation-mode gate — checked BEFORE Flows or Automations, not
  // just before the AI agent (which already gates itself internally,
  // see runAiAgent). Previously only the AI respected `mode`: a
  // keyword automation or a flow trigger could still auto-fire and
  // message the customer even after a human explicitly took over
  // (mode = 'human_active') or the conversation was paused/closed,
  // because neither engine ever read `mode` at all — executeHandoff
  // (flows/engine.ts) only ever WROTE it. Any step in either engine
  // can include a send_message action, so both must be gated, not
  // just the content-level triggers. ai_suggestion_only (Copilot) is
  // deliberately NOT gated here — that mode governs the AI's own
  // free-form replies, not the deterministic engines.
  // ============================================================
  const conversationMode = (conversation as { mode?: string }).mode ?? 'ai_active'
  const conversationIsHumanOwned =
  ...
  // ============================================================
  const flowResult = conversationIsHumanOwned
    ? { consumed: false }
  ...
  //
  // keyword_match is dispatched separately from the rest and AWAITED
  // (not fire-and-forget) — the AI agent fallback below needs to know
  // whether a deterministic rule already claimed the message before it
  // runs, same rationale as awaiting the Flows dispatch above.
  const inboundText = contentText ?? message.text?.body ?? ''
  const backgroundAutomationTriggers: (
    | 'new_contact_created'
    | 'first_inbound_message'
    | 'new_message_received'
  )[] = []
  let keywordAutomationMatched = false

  // Content-level triggers are suppressed when a flow consumed the
  // message — see the comment block above — or when a human owns
  // this conversation.
  if (!flowConsumed && !conversationIsHumanOwned) {
    backgroundAutomationTriggers.push('new_message_received')
    const keywordResult = await runAutomationsForTrigger({
      accountId,
      ...
      console.error('[automations] keyword_match dispatch failed:', err)
      return { matched: false }
    })
    keywordAutomationMatched = keywordResult.matched
  }
  ...
  if (contactOutcome.wasCreated) backgroundAutomationTriggers.unshift('new_contact_created')
  if (isFirstInboundMessage) backgroundAutomationTriggers.unshift('first_inbound_message')
  // Fire-and-forget: a slow or failing automation must not block the
  // webhook's 200 OK response to Meta. Still gated on conversationIsHumanOwned
  // — even a relationship-level trigger's automation can carry a
  // send_message step.
  if (!conversationIsHumanOwned) {
    for (const triggerType of backgroundAutomationTriggers) {
  ...
  // ============================================================
  // AI agent — the fallback path. Only reached when nothing
  // deterministic (a Flow or a keyword automation) already claimed the
  // message. Awaited, not fire-and-forget: it reads/writes the
  // per-conversation ai_sessions row, so two inbound messages arriving
  // close together must not race each other there — same reasoning as
  // awaiting the Flows dispatch above. The agent's own gating
  // (ai_agent_config.enabled, conversation.mode) makes this call a safe
  // no-op for any account that hasn't set up AI at all.
  // ============================================================
  if (!flowConsumed && !keywordAutomationMatched) {
    try {
      await runAiAgent({
        accountId,
        userId: configOwnerUserId,
