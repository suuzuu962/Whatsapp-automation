...
export async function POST(request: Request) {
  try {
    ...
    // Opted-out contacts must never receive a broadcast — this is
    // exactly the marketing-message case opt-out compliance exists
    // for. Broadcast recipients only carry a phone number (no
    // contact_id — see NewRecipient above), so match against every
    // opted-out contact on this account by phone rather than id.
