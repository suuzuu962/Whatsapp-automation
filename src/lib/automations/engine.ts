...
export interface DispatchResult {
  /**
   * True iff at least one active automation's trigger condition matched
   * for this event — regardless of whether its steps then succeeded.
   * The webhook uses this (for the `keyword_match` trigger specifically)
   * to decide whether a deterministic rule already claimed the message
   * before falling through to the AI agent: even a failed automation run
   * still means a human authored a rule for this exact keyword, so the
   * AI should not also reply.
   */
  matched: boolean
}
...
export async function runAutomationsForTrigger(
  ...
  try {
    ...
    if (!automations || automations.length === 0) return { matched: false }
