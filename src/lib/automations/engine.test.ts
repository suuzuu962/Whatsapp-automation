...
// The webhook (Phase 1 AI-core work) uses `matched` specifically for the
// keyword_match trigger to decide whether a deterministic automation
// already claimed the message before falling through to the AI agent.
describe("runAutomationsForTrigger — matched result", () => {
  it("reports matched:false when no automations exist for the trigger", async () => {
    h.state.owned = { id: "c1" };
    h.state.automations = [];
  ...
  it("reports matched:false when automations exist but none match the message text", async () => {
    h.state.owned = { id: "c1" };
    h.state.automations = [keywordAutomation(["pricing"])];
    h.state.steps = [];
    ...
    const result = await runAutomationsForTrigger({
      ...
      triggerType: "keyword_match",
      contactId: "c1",
      context: { message_text: "what time do you open" },
    });

    expect(result).toEqual({ matched: false });
  });

  it("reports matched:true when a keyword automation's condition matches", async () => {
    h.state.owned = { id: "c1" };
    h.state.automations = [keywordAutomation(["pricing"])];
    h.state.steps = [];
    ...
    const result = await runAutomationsForTrigger({
      ...
      triggerType: "keyword_match",
      contactId: "c1",
      context: { message_text: "what's your pricing?" },
    });

    expect(result).toEqual({ matched: true });
  });

  it("still reports matched:true even when the matched automation's steps fail", async () => {
    h.state.owned = { id: "c1" };
    h.state.automations = [keywordAutomation(["pricing"])];
    // A step type the mock's automation_steps lookup won't resolve
    // sensibly — the point is execution failing shouldn't change
    // `matched`, since a rule still claimed the message.
    h.state.steps = [
      { id: "bad", automation_id: "a-kw", step_type: "send_template", position: 0, parent_step_id: null, step_config: {} },
    ];
  ...
  it("reports matched:false when the contact ownership guard refuses dispatch", async () => {
    h.state.owned = null;
    h.state.automations = [keywordAutomation(["pricing"])];
