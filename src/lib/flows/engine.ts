...
async function executeHandoff(
  ...
  const convUpdate: Record<string, unknown> = {
    status: "pending",
    // Stops the AI agent from also replying once a flow hands off to a
    // human — mode is the single gate every send path checks.
    mode: "human_active",
...
async function handleReplyForActiveRun(
  ...
  if (action.type === "handoff") {
    if (run.conversation_id) {
      await db
        .from("conversations")
        // See executeHandoff — mode: "human_active" stops the AI agent
        // from also replying once fallback exhausts into a handoff.
        .update({
          status: "pending",
          mode: "human_active",
          updated_at: new Date().toISOString(),
